    ...
    methods: {
        ...
        submitData() {
            ...
            for (const fila of this.filas) {
                ...
                // Verificar si hay votos en blanco ingresados en al menos una columna
                const votosRecurridosIngresados =
                    this.votosRemitidos.intendente;

                // Verificar si hay votos en blanco ingresados en al menos una columna
                const votosImpugnadosIngresados =
                    this.votosImpugnados.intendente;

                if (votosNulosIngresados) {
                    // Agregar los votos nulos al objeto datosAgrupados
                    datosAgrupados["nulos"] = { votos: this.votosNulos };
                }

                if (votosBlancoIngresados) {
                    // Agregar los votos en blanco al objeto datosAgrupados
                    datosAgrupados["blanco"] = { votos: this.votosBlanco };
                }
                if (votosRecurridosIngresados) {
                    // Agregar los votos en blanco al objeto datosAgrupados
                    datosAgrupados["recurridos"] = { votos: this.votosRemitidos };
                }

                if (votosImpugnadosIngresados) {
                    // Agregar los votos en blanco al objeto datosAgrupados
                    datosAgrupados["impugnados"] = { votos: this.votosImpugnados };
                }
